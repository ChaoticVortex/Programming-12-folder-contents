...
for num in range(len(lines)):
	temp = lines[num].index('a')
	accel.append(lines[num][temp+3:-1].split(','))

for triple in range(len(accel)):
	for number in range(len(accel[triple])):
		accel[triple][number] = int(accel[triple][number])

lowest = abs(accel[0][0]) + abs(accel[0][1]) + abs(accel[0][2])
index = 0

for triple in accel:
	total = abs(triple[0]) + abs(triple[1]) + abs(triple[2])
	if total < lowest:
		lowest = total
